...
if ( !class_exists('Ftage_Video_Popup') ) {
	class Ftage_Video_Popup extends WP_Widget{

		function __construct(){

			$widget_options = array(
				'description' 					=> esc_html__('This widget show video thumbnail and video', 'ftagementor'), 
				'customize_selective_refresh' 	=> true,
			);

			parent:: __construct('Ftage_Video_Popup', esc_html__( 'Ftage: Video PopUp', 'ftagementor'), $widget_options );
            ...
            <div class="sidebar-video">
                <div class="video-img">
					...
					<?php if (!empty( $bg_image )): ?>
						<img src="<?php echo esc_url($bg_image); ?>" alt="<?php echo esc_attr('video popup bg image') ?>">
					<?php endif ?>
		...
		public function form($instance){ 
			...
			<div class="image_box_wrap" style="margin:20px 0 15px 0; width: 100%;">
				...
				<div class="image_box widefat">
					<img src="<?php if( !empty($bg_image)){echo esc_html($bg_image);} ?>" style="margin:15px 0 0 0;padding:0;max-width: 100%;display:inline-block; height: auto;" alt="<?php echo esc_attr('Image'); ?>" />
				</div>
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('video_link')); ?>"><?php echo esc_html__('Video Link:' ,'ftagementor') ?></label>
...
function Ftage_Video_Popup() {
    register_widget( 'Ftage_Video_Popup' );
}
add_action( 'widgets_init', 'Ftage_Video_Popup' );